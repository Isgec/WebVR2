Partial Class GF_vrUserRoles
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/VR_Main/App_Display/DF_vrUserRoles.aspx"
  ...
  End Sub
  Protected Sub GVvrUserRoles_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVvrUserRoles.RowCommand
		If e.CommandName.ToLower = "lgedit".ToLower Then
			Try
				Dim SerialNo As Int32 = GVvrUserRoles.DataKeys(e.CommandArgument).Values("SerialNo")  
				Dim RedirectUrl As String = TBLvrUserRoles.EditUrl & "?SerialNo=" & SerialNo
				Response.Redirect(RedirectUrl)
			Catch ex As Exception
			End Try
		End If
  End Sub
  Protected Sub GVvrUserRoles_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVvrUserRoles.Init
    DataClassName = "GvrUserRoles"
    SetGridView = GVvrUserRoles
  End Sub
  ...
  End Function
  Protected Sub F_DivisionID_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles F_DivisionID.SelectedIndexChanged
    Session("F_DivisionID") = F_DivisionID.SelectedValue
    InitGridPage()
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    End If
		...
		End If
		Dim validateScriptFK_VR_UserRoles_UserID As String = "<script type=""text/javascript"">" & _
			"  function validate_FK_VR_UserRoles_UserID(o) {" & _
			"    var value = o.id;" & _
			"    var UserID = $get('" & F_UserID.ClientID & "');" & _
			"    try{" & _
			"    if(UserID.value==''){" & _
			"      if(validated_FK_VR_UserRoles_UserID.main){" & _
			...
			"    PageMethods.validate_FK_VR_UserRoles_UserID(value, validated_FK_VR_UserRoles_UserID);" & _
			"  }" & _
			"  validated_FK_VR_UserRoles_UserID_main = false;" & _
			"  function validated_FK_VR_UserRoles_UserID(result) {" & _
			...
			"  }" & _
		  "</script>"
		If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_VR_UserRoles_UserID") Then
			Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_VR_UserRoles_UserID", validateScriptFK_VR_UserRoles_UserID)
		End If
  End Sub
	<System.Web.Services.WebMethod()> _
