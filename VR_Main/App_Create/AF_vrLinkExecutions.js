...
var script_vrLinkExecutions = {
		...
		validate_SRNNo: function(sender) {
		  ...
		  this.validated_FK_VR_LinkExecutions_SRNNo_main = true;
		  this.validate_FK_VR_LinkExecutions_SRNNo(sender,Prefix);
		  },
		...
		validate_FK_VR_LinkExecutions_SRNNo: function(o,Prefix) {
		  var value = o.id;
		  var SRNNo = $get(Prefix + 'SRNNo');
		  if(SRNNo.value==''){
		    if(this.validated_FK_VR_LinkExecutions_SRNNo_main){
		  ...
		  value = value + ',' + SRNNo.value ;
		    ...
		    o.style.backgroundRepeat= 'no-repeat';
		    o.style.backgroundPosition = 'right';
		    PageMethods.validate_FK_VR_LinkExecutions_SRNNo(value, this.validated_FK_VR_LinkExecutions_SRNNo);
		  },
		validated_FK_VR_LinkExecutions_SRNNo_main: false,
		validated_FK_VR_LinkExecutions_SRNNo: function(result) {
		  var p = result.split('|');
		  var o = $get(p[1]);
		  if(script_vrLinkExecutions.validated_FK_VR_LinkExecutions_SRNNo_main){
		    var o_d = $get(p[1]+'_Display');
		    try{o_d.innerHTML = p[2];}catch(ex){}
		  }
		  o.style.backgroundImage  = 'none';
